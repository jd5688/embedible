...
], function($, bootstrap, jcrypt, tooltip, _, Backbone, Contents_m, Content_m, DEM, Paginator, body_tpl, body_page_tpl, page_loading_tpl ){
	var Contents = {
		'View'	: function () { 
			return Backbone.View.extend({
				...
				render: function () {
					...
					var qry = this.model.get('qry');
					
					// remove unneccesary words in the query
					this._filter(qry);
					qry = encodeURIComponent(this._src_query);
					
					// render loading gif image
					var template = _.template( page_loading_tpl );
					this.$el.html( template );
					this.model.fetch({ url : DEM.domain + "search?hash=" + hash + "&publc=" + publc + "&limit=" + Paginator.limit + "&qry=" + qry + "&curPage=1&callback=?" }); // fetch data from the server
				...
				paginate: function (e) {
					...
					Contents.fetch({
						url : DEM.domain + "search?hash=" + hash + "&publc=" + publc + "&limit=" + Paginator.limit + "&curPage=" + Paginator.curPage + "&qry=" + qry + "&callback=?",
				...
				_src_query: '',
				_filter: function (qry) {
					var splt = '';
					var text = '';
					qry = qry.replace(/ is /g," ");
					qry = qry.replace(/ are /g," ");
					qry = qry.replace(/ the /g," ");
					qry = qry.replace(/ we /g," ");
					qry = qry.replace(/ were /g," ");
					qry = qry.replace(/ was /g," ");
					qry = qry.replace(/ have /g," ");
					qry = qry.replace(/ had /g," ");
					qry = qry.replace(/ you /g," ");
					qry = qry.replace(/ i /g," ");
					qry = qry.replace(/ was /g," ");
					qry = qry.replace(/ this /g," ");
					qry = qry.replace(/ these /g," ");
					qry = qry.replace(/ there /g," ");
					qry = qry.replace(/ his /g," ");
					qry = qry.replace(/ her /g," ");
					qry = qry.replace(/ him /g," ");
					text = qry.replace(/ /g,"|");
					this._src_query = text;
					return;
				},
